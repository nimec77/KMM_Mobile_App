...
fun CreateGameScreen() {
  ...
  ViewModel(factory = { CreateGameViewModel() }) { viewModel ->
    ...
    Column(modifier = Modifier.padding(16.dp)) {
      Text(
        text = viewState.name.ifBlank { "Add Game" },
      ...
      CommonTextField(
        ...
        viewModel.obtainEvent(CreateGameEvent.NameChanged(it))
      }
      ...
      CommonTextField(
        value = viewState.size.toString(),
        placeholder = "Game Size (Mb)",
        isSending = viewState.isSending,
      ) {
        viewModel.obtainEvent(CreateGameEvent.SizeChanged(it.toInt()))
      }
